...
class MintfileTests: XCTestCase {
    ...
    func testMintfileFromString() {
        ...
        XCTAssertEqual(
            mintfile.packages,
            [
                PackageReference(repo: "yonaskolb/xcodegen", version: PackageReference.Revision.branch("1.8.0")),
                PackageReference(repo: "realm/SwiftLint", version: PackageReference.Revision.branch("0.25.0")),
            ]
        )

        let expectedPackage = PackageReference(repo: "realm/SwiftLint", version: PackageReference.Revision.branch("0.25.0"))
    ...
    func testMintfileFromFile() throws {
        ...
        XCTAssertEqual(
            mintfile.packages,
            [
                PackageReference(repo: "yonaskolb/SimplePackage", version: PackageReference.Revision.branch("4.0.0")),
                PackageReference(repo: "yonaskolb/Mint", version: PackageReference.Revision.branch("0.9.1")),
            ]
        )

        let expectedPackage = PackageReference(repo: "yonaskolb/SimplePackage", version: PackageReference.Revision.branch("4.0.0"))
